...
%===================================================================%
ilastik_filename = 'Series008_Crop001_RAW_ch00_bin2_Probabilities_update2.h5'; % ilastik prediction map here
%===================================================================%
%% put here the image size info (in microns): read that from ImageJ/FIJI

%img_length = 246.27;
pixelSize =0.2403837;%img_length/512.0; % pixel size in microns for 512x512 image, or put here straight the pixel size
voxelDepth = 0.333;
...
%===================================================================%
imagename = 'Series008_Crop001_RAW_ch00_bin2.tif';   % membrane file name
%===================================================================%
...
%Limits are set in cubic microns, the voxel size estimate before is used
%for the conversion. 
lower_limit_in_cube_microns=50;
upper_limit_in_cube_microns=500;
lower_limit_in_cube_pixels=(round(lower_limit_in_cube_microns/voxelSize));
upper_limit_in_cube_pixels=(round(upper_limit_in_cube_microns/voxelSize));
%===================================================================%
Label2 = bwareaopen(Label,lower_limit_in_cube_pixels);             % min                   
Label2 = Label2 - bwareaopen(Label,upper_limit_in_cube_pixels);  % max   
%===================================================================%
...
for z = 1 : size(Final_Label,3)                                
    imwrite(Final_Label(:,:,z),'series008_Final_Label_2_seg4.tif','compression','none','writemode','append');
end                                                                
...
istats = regionprops3(Final_Label,'all');
%% 9.b Exclude part of the image with a dividing line (i.e. UB stalk exclusion)

fig=figure();
imshow(max(original_img,[],3),[])
title('click and hold to draw line. (selected cells are clockwise from first click)')
line=drawline();
origin=line.Position(1,:);
vec=line.Position(2,:)-line.Position(1,:);

vec=[vec,0];
for i = 1: size(istats.Centroid,1)
    cell_vec=[istats.Centroid(i,1:2)-origin,0];
    test=cross(vec,cell_vec);
    sig(i)=test(3)>0;
end
full_index=1:length(sig);
selected_index=full_index(sig);     
hold on
plot(istats.Centroid(sig,1),istats.Centroid(sig,2),'go')
title('Check that right set of cells is selected')
...
stats_matrix_all(:,1) = CellVolumes*voxelSize; % parameter 1: volume              
...
stats_matrix_all(:,5) = LongestAxis*pixelSize                   ;      % parameter 5: the length of the longest axis
stats_matrix_all(:,6) = stats.PrincipalAxisLength(:,2)*pixelSize
stats_matrix_all(:,7) = stats.PrincipalAxisLength(:,3)*pixelSize
...
for z = 1 : size(Label_sub,3)
    ...
    imwrite(temp,name,'tiff','Compression','packbits','WriteMode','append');
end
%% 
% 
%% Visualize single parameters in the volume viewer (runs quickly for visualization/thresholding)
%% can skip ahead to line 437 to run all parameters and save tifs
...
parameter=1 % select paramter here (1-5)

treshold_in_z_score=0; % change z-score threshold here


    stat_vector=zscored_matrix(:,parameter);
    full_index=1:length(stat_vector);
    %use > or < depending on larger or smaller stat required
    cells_of_interest=full_index(stat_vector'<treshold_in_z_score );
    cells_of_not_interest=full_index(stat_vector'>=treshold_in_z_score );

Label_sub = 0*Final_Label;  

  
    for i = 1 : length(cells_of_interest) 
        Label_sub(Final_Label == cells_of_interest(i)) = 1;
    end
    for i = 1 : length(cells_of_not_interest) 
        Label_sub(Final_Label == cells_of_not_interest(i)) = 2;
    end
...
treshold_in_z_score=0; % change z-score threshold here
    ...
    cells_of_interest=index(stat_vector'<treshold_in_z_score & sig==1); %use > or < depending on larger or smaller stat required
...
for z = 1 : size(Label_sub,3)
    ...
    for c = 1 : length(cells_of_interest)
        ...
        tmp = tmp>0;
        temp(:,:,1) = uint8(cmp(3,1).*255*double(tmp))+temp(:,:,1);
        temp(:,:,2) = uint8(cmp(3,2).*255*double(tmp))+temp(:,:,2);
        temp(:,:,3) = uint8(cmp(3,3).*255*double(tmp))+temp(:,:,3);
    end
    imwrite(temp,[param_name{parameter},'_small.tif'],'tif','Compression','packbits','WriteMode','append'); %change '_small.tif' to '_large.tif' when viewing the larger populations
end
...
vol=[];
name=[]
parameter=1 %input which parameter you want to plot (1-5)
for folde = 1:length(folders)
    load(fullfile(root_path,folders{folde},'stats_matrix_all'));
    name=[name;repmat(folders{folde},size(stats_matrix_all,1),1)]
    vol=[vol;stats_matrix_all(:,parameter)];
end
boxplot(vol,name)
%%
counter = 0;
...
%% 13: multiple samples in same heatmap
...
xp_ref_num = 1; % type here the embryo number 1)E11-1  2)E11  3)3c2  4)4a1  5)4b2
counter    = 5;  % this pointer is for the color
indices = 1498; % group number from heatmap
...
%% EXTRA: compare the cells with two fluorescent signal and visualize the double positive signal cells
...
% this only a function, can be ignored by the user. Credit of this is for this 
% function is for the SGA code makers
