%% Visualize the joint heatmap of parameter matrices without a threshold

root_path = ''; % insert file path to a root folder containing subfolders of all tips
folders   = {'Ampulla 1', 'Ampulla 2', 'Ampulla 3', 'Asymmetrical ampulla 1', 'Asymmetrical ampulla 2', 'Asymmetrical ampulla 3', 'Bifurcated tip 1', 'Bifurcated tip 2', 'Bifurcated tip 3', 'Initial bud 1', 'Initial bud 2', 'Initial_bud 3_5', 'Initial_bud 4_5' }; % folders here (you need save combined stats_matrix_all for each group, ex. ampullae, bifurcated, etc. into separate folders)
counter = 0;
...
params     = {'Volume','Roundness','Ellipticity','Elongation','Longest Axis'};
...
for xp_ref_num = 1:length(folders); % type here the embryo number 1)E11-1  2)E11  3)3c2  4)4a1  5)4b2
counter    = 1;  % this pointer is for the color
indices = 3722; % group number from heatmap
tiffname = '3722'; %write the name of pseudocolored cell group that we will save as tiff
%=========================================================================
...
for c = 1 : length(sub_cluster_BIG_pruned)
    ...
    for i = 1 : length(cells_of_Interest) 
        ...
        num_cells_of_interest(xp_ref_num) = length(cells_of_Interest);
    end
end
for z = 1 : size(Label_sub,3)
 ...
 imwrite(temp,['869_',num2str(xp_ref_num),'.tif'],'tiff','Compression','none','WriteMode','append');
end
